...
async function tryMonkey(label, getMethod) {
	...
	if (label === 'fs.realpath' || label === 'fs.realpathSync') {
		assert.notEqual(monkeyed.native, unmonkeyed.native, `${label}.native should be different after monkeying`);
		assert.equal(original.native, unmonkeyed.native, `${label}.native should be restored after unmonkeying`);
	}
}
